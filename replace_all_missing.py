...
# fetchBoards
text = re.sub(
    r'''    public CompletableFuture<List<JiraBoard>> fetchBoards\(\) \{[\s\S]*?\}\s*\}\);[\n\r]*    \}''',
...
# fetchTasks (Long boardId)
text = re.sub(
    r'''    public CompletableFuture<List<JiraTask>> fetchTasks\(Long boardId\) \{[\s\S]*?\}\s*\}\);[\n\r]*    \}''',
...
text = re.sub(
    r'''    public CompletableFuture<Boolean> assignTaskToCurrentUser\(String issueKey\) \{[\s\S]*?\}\s*\}\);[\n\r]*    \}''',
