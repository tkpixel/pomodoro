...
/**
 * Manages secure storage of secrets using the native OS Credential Manager (Windows Credential
 * Manager on Windows) via java-keyring.
 *
 * <p>Non-sensitive plaintext values (e.g. URLs, email addresses) continue to use
...
public class SecretManager {
  ...
  public SecretManager() {
    ...
    try {
      ...
      log.warn(
          "Native Keyring backend not available, falling back to in-memory stub. Secrets will NOT persist.",
          e);
      k = null;
    }
    this.keyring = k;
  ...
  public void saveSecret(String key, String secretValue) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to save secret to Windows Credential Manager for key: " + key, e);
    }
  }
