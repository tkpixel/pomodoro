...
public class JiraBoardViewModel {
  ...
  private final ObservableMap<String, ObservableList<TaskCardViewModel>> columnTasksMap =
      FXCollections.observableHashMap();
  private final ObservableMap<String, BooleanProperty> columnVisibilityMap =
      FXCollections.observableHashMap();
                    ...
                    () -> {
                      ...
                      if (savedBoardIdStr != null && !savedBoardIdStr.isBlank()) {
                        try {
                          ...
                          Optional<BoardViewModel> boardToSelect =
                              boards.stream().filter(b -> b.id().equals(savedBoardId)).findFirst();
  ...
  private void fetchBoardConfigurationAndTasks(Long boardId) {
    isLoading.set(true);
    jiraBoardService
        .fetchBoardConfiguration(boardId)
        .thenCompose(
            config -> {
              Platform.runLater(
                  () -> {
  ...
  public void assignTaskToCurrentUser(String taskKey) {
    isLoading.set(true);
    jiraBoardService
        .assignTaskToCurrentUser(taskKey)
        .thenAccept(
            success ->
                Platform.runLater(
                    () -> {
  ...
  private void moveTask(JiraTask task, String targetColumnName) {
    ...
    isLoading.set(true);
    jiraBoardService
        .fetchTransitions(task.key())
        .thenCompose(
            transitions -> {
              Optional<JiraTransition> validTransition =
                  transitions.stream()
              ...
              if (validTransition.isPresent()) {
                ...
                throw new RuntimeException(
                    "No valid transition found to move " + task.key() + " to " + targetColumnName);
              }
            })
        .thenAccept(
            success ->
  ...
  public ObservableList<BoardViewModel> getBoards() {
    return boards;
  }

  public ObjectProperty<BoardViewModel> selectedBoardProperty() {
    return selectedBoard;
  }

  public BoardViewModel getSelectedBoard() {
    return selectedBoard.get();
  }

  public void setSelectedBoard(BoardViewModel board) {
    this.selectedBoard.set(board);
  }

  public ObservableList<String> getDynamicColumnNames() {
    return dynamicColumnNames;
  }

  public ObservableMap<String, ObservableList<TaskCardViewModel>> getColumnTasksMap() {
    return columnTasksMap;
  }

  public BooleanProperty getColumnVisibilityProperty(String columnName) {
    return columnVisibilityMap.get(columnName);
  }
