...
public class CreateIssueViewModel {
  ...
  public void initData() {
    ...
    // Jira Boards typically have board ID != project ID, but in Jira Software Cloud, boards are
    // often tightly coupled to a project context.
    // For basic creation without complex context fetching, we attempt to use the board ID as the
    // project ID, or fetch the first project.
    // Here we'll try to just pass the board ID. Realistically, we'd fetch the project for the
    // board. We'll use selected board id for now.
    // It's a limitation we document or assume board id maps nicely or is required to create via
    // board.
    ...
    jiraBoardService
        .fetchIssueTypes(currentProjectId)
        .thenAccept(
            issueTypes -> {
              Platform.runLater(
                  () -> {
        ...
        .thenCompose(v -> jiraBoardService.fetchPriorities())
        .thenAccept(
            priorities -> {
              Platform.runLater(
                  () -> {
  ...
  public void submitIssue() {
    ...
    if (!originalEstimate.get().trim().isEmpty()) {
      timetracking =
          new JiraTask.Timetracking(originalEstimate.get().trim(), null, null, null, null, null);
    }
  ...
  public StringProperty originalEstimateProperty() {
  ...
  public ObjectProperty<IssueType> selectedIssueTypeProperty() {
  ...
  public BooleanProperty createAnotherProperty() {
    return createAnother;
  }

  public ObservableList<IssueType> getAvailableIssueTypes() {
    return availableIssueTypes;
  }

  public ObservableList<Priority> getAvailablePriorities() {
