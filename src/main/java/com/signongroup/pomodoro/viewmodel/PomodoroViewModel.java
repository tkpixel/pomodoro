...
/** ViewModel für die Hauptansicht (MVVM-Pattern). Wird von Micronaut als Singleton verwaltet. */
@Singleton
public class PomodoroViewModel {
  ...
  private final ObjectProperty<TimerState> timerState =
      new SimpleObjectProperty<>(TimerState.READY);
  ...
  public PomodoroViewModel(
    ...
    this.settingsViewModel
        .shortBreakMinutesProperty()
        .addListener((obs, oldVal, newVal) -> updateSettingsFromViewModel());
    this.settingsViewModel
        .longBreakMinutesProperty()
        .addListener((obs, oldVal, newVal) -> updateSettingsFromViewModel());
    this.settingsViewModel
        .maxSessionCountProperty()
        .addListener(
            (obs, oldVal, newVal) -> {
              updateSettingsFromViewModel();
              updateUI();
    ...
    this.trackingService
        .activeModeProperty()
        .addListener((obs, oldVal, newVal) -> pushTrackingState());
  ...
  private void tick() {
    if (timeRemainingSeconds > 0) {
      ...
      if (timeRemainingSeconds == 5) {
        ...
        } else if ((timerState.get() == TimerState.BREAK_SHORT
                || timerState.get() == TimerState.BREAK_LONG)
  ...
  private void handleTimerComplete() {
    ...
    if (timerState.get() == TimerState.FOCUS_RUNNING) {
      if (activeTaskService.getActiveTask() != null) {
        jiraBoardService.addWorklog(
            activeTaskService.getActiveTask().taskKeyProperty().get(), focusTimeSeconds);
      ...
      if (timerState.get() == TimerState.BREAK_SHORT && activeTaskService.getActiveTask() != null) {
        jiraBoardService.addWorklog(
            activeTaskService.getActiveTask().taskKeyProperty().get(), shortBreakSeconds);
  ...
  private void updateUI() {
    if (timerState.get() == TimerState.BREAK_SHORT || timerState.get() == TimerState.BREAK_LONG) {
      int maxTime =
          (timerState.get() == TimerState.BREAK_LONG) ? longBreakSeconds : shortBreakSeconds;
  ...
  public void resetCurrentPhase() {
    ...
    if (currentState == TimerState.BREAK_SHORT || currentState == TimerState.BREAK_LONG) {
      int maxTime =
          (currentSession == 1 && clearedToday > 0) ? longBreakSeconds : shortBreakSeconds;
