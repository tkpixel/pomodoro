...
public class TaskCardComponent extends VBox {
  ...
  public TaskCardComponent(
      ...
      PomodoroViewModel mainViewModel,
      JiraBoardViewModel jiraBoardViewModel) {
  ...
  private void setupInteractivity() {
    this.setOnMouseEntered(
        e -> {
          if (!viewModel.isCompletedProperty().get()) {
            this.setStyle(
                "-fx-background-color: #1C1C1E; -fx-background-radius: 16; -fx-padding: 20; -fx-border-color: #494847; -fx-border-radius: 16; -fx-border-width: 1; -fx-cursor: hand;");
          }
        });
    this.setOnMouseExited(
        e -> {
          if (!viewModel.isCompletedProperty().get()) {
            this.setStyle(
                "-fx-background-color: #1C1C1E; -fx-background-radius: 16; -fx-padding: 20; -fx-border-color: transparent; -fx-border-radius: 16; -fx-border-width: 1; -fx-cursor: hand;");
          }
        });
    ...
    this.setOnContextMenuRequested(
        e -> {
          if (jiraBoardViewModel != null) {
            ...
            for (java.util.Map.Entry<String, javafx.collections.ObservableList<TaskCardViewModel>>
                entry : jiraBoardViewModel.getColumnTasksMap().entrySet()) {
            ...
            if (currentColumn != null) {
              java.util.List<String> adjacentColumns =
                  jiraBoardViewModel.getAdjacentColumnNames(currentColumn);
  ...
  private void bindData() {
    ...
    if (viewModel.issueTypeIconUrlProperty().get() != null
        && !viewModel.issueTypeIconUrlProperty().get().isEmpty()) {
      try {
    ...
    viewModel
        .priorityIconColorProperty()
        .addListener(
            (obs, oldVal, newVal) -> {
              if (newVal != null && !newVal.isEmpty()) {
                try {
                  priorityIcon.setIconColor(javafx.scene.paint.Color.web(newVal));
                } catch (Exception ignored) {
                }
              }
            });
    if (viewModel.priorityIconColorProperty().get() != null) {
      try {
        priorityIcon.setIconColor(
            javafx.scene.paint.Color.web(viewModel.priorityIconColorProperty().get()));
      } catch (Exception ignored) {
      }
    }
  ...
  private void applyStateStyling() {
    if (viewModel.isCompletedProperty().get()) {
      this.setOpacity(0.6);
      taskTitle.setStyle(
          "-fx-font-size: 16px; -fx-font-weight: bold; -fx-text-fill: -fx-on-surface-variant; -fx-strikethrough: true;");
      taskKey.setStyle(
          "-fx-font-size: 10px; -fx-text-fill: -fx-on-surface-variant; -fx-strikethrough: true;");
        ...
        progressBarFill
            ...
            .bind(
                progressContainer
                    .widthProperty()
                    .subtract(32)
                    .multiply(viewModel.progressProperty()));
