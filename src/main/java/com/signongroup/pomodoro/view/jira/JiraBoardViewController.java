...
public class JiraBoardViewController implements Initializable {
  ...
  @Inject
  public JiraBoardViewController(
      JiraBoardViewModel viewModel, WindowManager windowManager, PomodoroViewModel mainViewModel) {
    ...
    viewModel
        ...
        .addListener(
            (javafx.collections.MapChangeListener.Change<
                        ? extends String,
                        ? extends javafx.collections.ObservableList<TaskCardViewModel>>
                    change) -> {
              if (change.wasAdded()) {
                ...
                change
                    .getValueAdded()
                    .addListener(
                        (ListChangeListener.Change<? extends TaskCardViewModel> c) -> {
  ...
  private void setupBoardComboBox() {
    ...
    boardComboBox.setCellFactory(
        listView ->
            new ListCell<BoardViewModel>() {
              @Override
  ...
  private void rebuildColumnsUI() {
    ...
    for (String colName : viewModel.getDynamicColumnNames()) {
      ...
      title.setStyle(
          "-fx-font-family: 'Manrope'; -fx-font-weight: bold; -fx-text-fill: "
              + viewModel.getColorForColumn(colName)
              + "; -fx-font-size: 12px; -fx-text-transform: uppercase;");

      Label countBadge = new Label("0");
      countBadge.setStyle(
          "-fx-background-color: -fx-surface-container-highest; -fx-text-fill: -fx-on-surface-variant; -fx-font-size: 10px; -fx-padding: 2 6; -fx-background-radius: 10;");

      // Bind count
      if (viewModel.getColumnTasksMap().containsKey(colName)) {
        countBadge
            .textProperty()
            .bind(Bindings.size(viewModel.getColumnTasksMap().get(colName)).asString());
      }
    ...
    Platform.runLater(
        () -> {
          ...
          for (TaskCardViewModel taskVM : tasks) {
            TaskCardComponent card =
                new TaskCardComponent(taskVM, windowManager, mainViewModel, viewModel);
