...
public class SettingsViewPage {
    ...
    public SettingsViewPage clickIncFocus() {
        robot.lookup("#focusSessionLabel").queryAs(Label.class).getParent().getChildrenUnmodifiable().stream()
    ...
    public SettingsViewPage clickIncMaxSessions() {
        robot.lookup("#maxSessionsLabel").queryAs(Label.class).getParent().getChildrenUnmodifiable().stream()
                .filter(node -> node instanceof javafx.scene.control.Button btn && btn.getGraphic() instanceof FontIcon icon && "fltfal-add-20".equals(icon.getIconLiteral()))
                .findFirst().ifPresent(robot::clickOn);
        return this;
    }
